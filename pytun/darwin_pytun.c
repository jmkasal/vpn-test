...
static int pytun_tuntap_set_addr(PyObject *self, PyObject *value, void *d) {
    ...
    // Command to set the address (using ifconfig or equivalent)
    sprintf(cmd, "ifconfig %s inet %s", tuntap->name, addr);
    if (system(cmd) != 0) {
        ret = -1;
        goto out;
    }
...
static PyObject * pytun_tuntap_set_config(PyObject *self, PyObject *args, PyObject *kwds) {
    printf("set_config called\n");

    pytun_tuntap_t *tuntap = (pytun_tuntap_t *) self;
    printf("tun_tap name: %s\n", tuntap->name);
    ...
    static char *kwlist[] = {"src_addr", "dst_addr", "netmask", NULL};

    // Parse the arguments: source address, destination address, and netmask
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "sss", kwlist, &src_addr, &dst_addr, &netmask)) {
//        PyErr_SetString(PyExc_ValueError, "Failed to parse arguments. Expected three strings.");
        raise_error_from_errno();
        ret = -1;
    ...
    if (src_addr == NULL || dst_addr == NULL || netmask == NULL) {
//        PyErr_SetString(PyExc_ValueError, "Source address, destination address, and netmask are required.");
        raise_error_from_errno();
        ret = -1;
        return PyLong_FromLong(ret);
    }

    // Build the command to configure the TUN interface
    printf("src_addr: %s, dst_addr: %s, netmask: %s\n", src_addr, dst_addr, netmask);

    snprintf(cmd, sizeof(cmd), "ifconfig %s %s %s netmask %s", tuntap->name, src_addr, dst_addr, netmask);

    // Execute the system command

    printf("trying to execute ifconfig command:\n %s\n", cmd);
    if (system(cmd) != 0) {
//        PyErr_SetString(PyExc_RuntimeError, "Failed to configure the TUN interface with the provided parameters.");
        raise_error_from_errno();
        ret = -1;
        return PyLong_FromLong(ret);
    }
    printf("ifconfig command executed successfully\n");
//    sleep(10);
    return PyLong_FromLong(ret);;
}
...
static PyMethodDef pytun_tuntap_meth[] =
        {
                ...
                {
                        "set_config",
                        (PyCFunction) pytun_tuntap_set_config,
